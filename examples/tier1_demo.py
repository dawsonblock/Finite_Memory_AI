...
def demo_tier1_semantic():
    ...
    for i, msg in enumerate(conversation, 1):
        ...
        if llm._span_embedder:
            ...
            print(f"  → Embedding cache: {cache_stats['cache_size']} entries, "
                  f"{cache_stats['hit_rate']:.1%} hit rate")
        
        print()
    
    print("=" * 70)
    print(f"Final stats: {llm.stats.tokens_seen} tokens seen, "
...
def demo_tier1_latency_budget():
    ...
    for i in range(5):
        ...
        stats = result['stats']
        print(f"Turn {i+1}: Policy took {stats.policy_latency_ms:.1f}ms, "
