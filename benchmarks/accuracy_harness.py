...
def evaluate_policy(
    ...
    overall_accuracy = sum(
        sum(results) for results in results_by_position.values()
    ) / sum(len(results) for results in results_by_position.values())
...
def compare_policies(
    ...
    print("="*70)
    print(f"{'Policy':<15} {'Accuracy':<12} {'Early':<10} {'Mid':<10} {'Late':<10} {'Compression':<12}")
    print("-"*70)
    ...
    for r in results:
        ...
        print(f"{r['policy']:<15} {acc:>6.1%}        {early:>5.1%}      {mid:>5.1%}      {late:>5.1%}      {comp:>6.2f}x")
    ...
    if results:
        ...
        print(f"📊 Best Compression: {best_compression['policy']} ({best_compression['compression_ratio']:.2f}x)")
