...
class PolicyBenchmark:
    ...
    def print_comparison_table(self, results: List[Dict[str, Any]]):
        ...
        # Compression metrics
        print("\n--- Compression ---")
        print(f"{'Policy':<20} {'Tokens Seen':<15} {'Retained':<15} {'Ratio':<15} {'Evictions':<15}")
        print("─" * 70)
        for r in results:
            print(f"{r['policy']:<20} "
                  f"{r['tokens_seen']:<15} "
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=512,
        help="Max tokens for memory (default: 512)"
    )
    parser.add_argument(
        "--model",
        default="gpt2",
