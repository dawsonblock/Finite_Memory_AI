...
def example_backends():
    ...
    backends = {
        "Cohere": lambda: CohereBackend(
            api_key="your-cohere-key", model="command", tokenizer=tokenizer
        ),
        "AI21": lambda: AI21Backend(api_key="your-ai21-key", model="j2-ultra", tokenizer=tokenizer),
        "Anthropic": lambda: AnthropicBackend(
            api_key="your-anthropic-key", model="claude-3-sonnet-20240229", tokenizer=tokenizer
        ),
        "Google": lambda: GoogleBackend(
            api_key="your-google-key", model="gemini-pro", tokenizer=tokenizer
        ),
    }
