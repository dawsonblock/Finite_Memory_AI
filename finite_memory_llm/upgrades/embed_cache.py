...
try:
    from sklearn.cluster import MiniBatchKMeans as MiniBatchKMeansType  # type: ignore[no-redef]
except ImportError:
    pass
...
class SpanEmbedder:
    ...
    def encode_spans(self, spans: List[List[int]], texts: Optional[List[str]] = None) -> np.ndarray:
    ...
    def select_representatives(
        self, embeddings: np.ndarray, k: int, recency_bias: float = 0.15
    ) -> List[int]:
        ...
        """
        if MiniBatchKMeansType is None:
            raise ImportError(
        ...
        can_warm_start = (
            self._kmeans is not None
            and self._last_k == k
            and hasattr(self._kmeans, "cluster_centers_")
        )

        if can_warm_start and self._kmeans is not None:
            # Warm-start: continue from previous clustering
            kmeans = self._kmeans
            kmeans.partial_fit(embeddings)
        else:
            # Cold start: new clustering
            kmeans = MiniBatchKMeansType(
                n_clusters=k,
        ...
        for cluster_id in range(k):
            ...
            if recency_bias > 0:
                ...
                combined_scores = (1 - norm_dist) * (
                    1 - recency_bias
                ) + recency_scores * recency_bias
