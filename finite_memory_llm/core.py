...
try:
    ...
    guarded_call = None  # type: ignore
    timed_call = None  # type: ignore
    SpanEmbedder = None  # type: ignore
    SummaryQAGate = None  # type: ignore
    choose_under_budget = None  # type: ignore
    partition_budget = None  # type: ignore
    Metrics = None  # type: ignore
    TurnDumper = None  # type: ignore
    build_block_sparse_mask = None  # type: ignore
    export_longformer_mask = None  # type: ignore
...
class TelemetryHook:
    ...
    def on_policy_execute(
        self, policy: str, latency_ms: float, tokens_before: int, tokens_after: int
    ) -> None:
    ...
    def __init__(self) -> None:
        try:
            ...
            self.policy_latency = Histogram(
                "finite_memory_policy_latency_seconds", "Policy execution latency", ["policy"]
            )
            self.tokens_evicted = Counter(
                "finite_memory_tokens_evicted_total", "Total tokens evicted"
            )
...
class HuggingFaceBackend(LLMBackend):
    ...
    def __init__(self, model_name: str = "gpt2", device: str = "cpu", enable_kv_cache: bool = True):
        ...
        self.model = AutoModelForCausalLM.from_pretrained(model_name).to(device)  # type: ignore
    ...
    def generate_stream(self, input_ids: torch.Tensor, max_new_tokens: int, **kwargs: Any) -> Any:
            ...
            for i in range(max_new_tokens):
                if generated_tokens:
                    ...
                    yield {
                        ...
                        "is_final": (
                            next_token == self.tokenizer.eos_token_id or i == max_new_tokens - 1
                        ),
                    }
...
class APIChatBackend(LLMBackend):
    ...
    def __init__(
        self, tokenizer: Any, send_callable: Callable[[str, int], str], name: str = "api-chat"
    ) -> None:
        self._tok = tokenizer
    ...
    def encode(self, text: str) -> list[int]:
        """Encode text to token IDs."""
        return self._tok.encode(text, add_special_tokens=False)  # type: ignore

    def decode(self, tokens: list[int]) -> str:
        """Decode token IDs to text."""
        return self._tok.decode(tokens, skip_special_tokens=True)  # type: ignore
...
class ContextBuilder:
    ...
    def global_boundaries(self, backend: LLMBackend, toks: list[int]) -> list[int]:
    ...
    def build(
        self, backend: LLMBackend, buffer: list[int], policy_out: list[int]
    ) -> tuple[list[int], int]:
...
class CompleteFiniteMemoryLLM:
    ...
    def __init__(
        ...
        # Memory state
        self.token_buffer: Any = deque(maxlen=max_tokens)
        self.attention_scores: Any = deque(maxlen=max_tokens)

        # Semantic policy state
        self.token_embeddings: list[Any] = []
        self.span_texts: list[str] = []

        # Rolling summary state
        self.summary_tokens: list[int] = []
        ...
        print(
            f"✓ LLM initialized: policy={self.memory_policy}, max_tokens={self.max_tokens}, window={self.window_size}{budget_str}{upgrade_str}"
        )
    ...
    def _collect_last_token_importance(self, ctx_ids: list[int]) -> np.ndarray | None:
        ...
        try:
            ...
            return att  # type: ignore  # shape [seq]
        except Exception:
            return None

    def _importance_via_logit_probes(
        self, ctx_ids: list[int], n_probes: int = 8, span_size: int = 32
    ) -> np.ndarray:
    ...
    def _evict_importance(self, new_tokens: list[int]) -> list[int]:
        ...
        if len(self.attention_scores) == len(cur_list) and importance_budget > 0:
            ranked = sorted(
                range(len(cur_list)), key=lambda i: self.attention_scores[i], reverse=True
            )
        ...
        self.attention_scores = deque(
            [self.attention_scores[i] for i in sorted(keep)], maxlen=self.max_tokens
        )
        self.token_buffer.extend(new_tokens)
        self.attention_scores.extend([0.0] * n_new)
        return list(self.token_buffer)
    ...
    def _compute_span_embeddings(
        self, tokens: list[int], span_size: int = 64, stride: int = 32
    ) -> None:
    ...
    def _evict_semantic(self, new_tokens: list[int]) -> list[int]:
        ...
        if self._span_embedder:
            ...
            try:
                ...
                keep_span_indices = self._span_embedder.select_representatives(
                    embs, k=self.semantic_clusters, recency_bias=0.15
                )
                ...
                if choose_under_budget is not None:
        ...
        try:
            ...
            for span_idx in sorted(keep_spans):
                s_pos, e_pos, _ = self.token_embeddings[span_idx]
                s_final = max(0, min(s_pos, len(cur)))
                e_final = max(s_final, min(e_pos, len(cur)))
                new_buf.extend(cur[s_final:e_final])
    ...
    def _create_summary(self, tokens: list[int], max_summary_tokens: int = 128) -> list[int]:
    ...
    def _evict_rolling_summary(self, new_tokens: list[int]) -> list[int]:
        ...
        if (self.tokens_since_summary >= self.summary_interval) and (
            len(self.token_buffer) > self.summary_interval
        ):
            ...
            summary = self._create_summary(
                to_sum, max_summary_tokens=min(128, self.max_tokens // 8)
            )
            ...
            if len(self.summary_tokens) > self.max_tokens // 4:
                self.summary_tokens = self._create_summary(
                    self.summary_tokens, max_summary_tokens=self.max_tokens // 8
                )
    ...
    def _apply_policy(self, new_tokens: list[int]) -> list[int]:
        ...
        if guarded_call is not None:
            result = guarded_call(
                ...
                budget_ms=int(self.max_policy_ms),
        ...
        try:
            ...
            if elapsed_ms > self.max_policy_ms:
                print(
                    f"⚠ Policy '{self.memory_policy}' exceeded budget ({elapsed_ms:.1f}ms > {self.max_policy_ms}ms), falling back to sliding"
                )
    ...
    def chat(self, message: str, max_new_tokens: int = 50) -> dict[str, Any]:
        ...
        try:
            ...
            self.stats.compression_ratio = self.stats.tokens_seen / max(
                self.stats.tokens_retained, 1
            )

            # build the final enforced context
            context_tokens, anchor_hits = self.context_builder.build(
                self.backend, list(self.token_buffer), policy_out
            )
            ...
            self.conversation_history.append(
                {"role": "user", "content": message, "tokens": str(len(msg_tokens))}
            )
            self.conversation_history.append(
                {"role": "assistant", "content": response_text, "tokens": str(len(gen_ids))}
            )
    ...
    def load_checkpoint(self, path: str | Path) -> dict[str, Any]:
        ...
        self.attention_scores = deque(
            checkpoint["state"]["attention_scores"], maxlen=self.max_tokens
        )
        ...
        return checkpoint["config"]  # type: ignore
...
def run_comprehensive_tests() -> None:
    ...
    for turn, msg in enumerate(convo, 1):
        ...
        print(
            f"   context_len={out['context_length']}  seen={out['stats'].tokens_seen}  kept={out['stats'].tokens_retained}"
        )
